...
def test_split_transform_invalid_parameters():
    erroneus_value = 0
    with pytest.raises(ValueError, match=f"Expected input 'num_splits' to be a positive integer greater than 0, but received {erroneus_value}."):
    ...
    erroneus_value = -1
    with pytest.raises(ValueError, match=f"Expected input 'split_dimension' to be a positive integer greater than or equal to 0, but received {erroneus_value}."):
        # Define an invalid split transform
...
def test_split_transform_invalid_input():
    ...
    with pytest.raises(TypeError, match=f"Expected input 'x' to be either a numpy array or a Pytorch tensor, but received an object of type {type(erroneus_value)}."):
...
def test_split_transform_invalid_split():
    ...
    # Define a valid split transform
    split_transform = SplitTransform(num_splits=3, split_dimension=1)
    with pytest.raises(ValueError, match=f"Invalid split: expected {split_transform.num_splits} to divide equally the dimension {random_input.shape[split_transform.split_dimension]}."):
    ...
    with pytest.raises(ValueError, match=f"Invalid split dimension: expected the split dimension to be less than {len(random_input.shape)}, but received {split_transform.split_dimension}."):
        # Apply the split transform with an invalid split
        split_transform(random_input)
