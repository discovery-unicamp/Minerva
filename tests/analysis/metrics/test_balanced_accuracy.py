...
def test_balanced_accuracy():
    ...
    assert torch.isclose(
        metric.compute().float(), torch.tensor(expected).float()
    ), "Test case 1 failed"
    ...
    assert torch.isclose(
        metric.compute().float(), torch.tensor(expected).float()
    ), f"Test case 6 failed with result {metric.compute()}"
    ...
    assert torch.isclose(
        metric.compute().float(), torch.tensor(expected).float()
    ), f"Test case 7 failed with result {metric.compute()}"
    ...
    assert torch.isclose(
        metric.compute().float(), torch.tensor(expected).float()
    ), f"Test case 8 failed with result {metric.compute()}"
    ...
    assert torch.isclose(
        metric.compute().float(), torch.tensor(expected).float()
    ), f"Test case 9 failed with result {metric.compute()}"
