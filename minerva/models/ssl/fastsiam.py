...
class SimSiamMLPHead(nn.Sequential):
    def __init__(
        ...
        *args, **kwargs : 
        ...
        assert all(isinstance(ls, int) and ls > 0 for ls in layer_sizes), "Layer sizes must be positive integers"
        assert issubclass(activation_cls, nn.Module), "activation_cls must inherit from torch.nn.Module"
...
class FastSiam(L.LightningModule):
    ...
    def __init__(
        ...
        self.prediction_branch_projector = SimSiamMLPHead([in_dim, 512, out_dim], activation_cls=nn.ReLU, batch_norm=True, final_bn=False, final_relu=False)
        self.prediction_branch_predictor = SimSiamMLPHead([out_dim, hid_dim, out_dim], activation_cls=nn.ReLU, batch_norm=True, final_bn=False, final_relu=False)

        # Target branch (momentum updated)
        self.target_branch_backbone = copy.deepcopy(backbone)
        self.target_branch_projector = SimSiamMLPHead([in_dim, hid_dim, out_dim], activation_cls=nn.ReLU, batch_norm=True, final_bn=False, final_relu=False)
    ...
    def update_target_branch(self) -> None:
        ...
        for prediction_branch_param, target_branch_param in zip(self.backbone.parameters(), self.target_branch_backbone.parameters()):
            target_branch_param.data = self.momentum * target_branch_param.data + (1.0 - self.momentum) * prediction_branch_param.data
        for prediction_branch_param, target_branch_param in zip(self.prediction_branch_projector.parameters(), self.target_branch_projector.parameters()):
            target_branch_param.data = self.momentum * target_branch_param.data + (1.0 - self.momentum) * prediction_branch_param.data
    ...
    def forward(self, views: Any) -> tuple[torch.Tensor, torch.Tensor]:
        ...
        prediction_branch_features = torch.flatten(prediction_branch_features, start_dim=1)
        prediction_branch_projected = self.prediction_branch_projector(prediction_branch_features)
        prediction_branch_predicted = self.prediction_branch_predictor(prediction_branch_projected)
        ...
        with torch.no_grad():
            for i in range(self.K):
                ...
                target_branch_features = torch.flatten(target_branch_features, start_dim=1)
                target_branch_projected = self.target_branch_projector(target_branch_features)
    ...
    def _single_step(self, batch: Any, K: int, log_prefix: str) -> torch.Tensor:
        ...
        for i in range(K + 1):
            ...
            with torch.no_grad():
                for j in range(K + 1):
                    if j != i:
                        target_branch_features = self.target_branch_backbone(augmented_views[j])
                        target_branch_features = self.global_avg_pool(target_branch_features)
                        target_branch_features = torch.flatten(target_branch_features, start_dim=1)
                        target_branch_projected = self.target_branch_projector(target_branch_features)
                        target_branch_projected_list.append(target_branch_projected)
            ...
            avg_target_branch_projected = F.normalize(avg_target_branch_projected, dim=-1, p=2)

            # Compute loss for the current view
            loss = self.fastsiam_loss(prediction_branch_predicted, avg_target_branch_projected.detach())
            total_loss += loss
        ...
        # Log the loss
        self.log(f"{log_prefix}_loss", total_loss, batch_size=len(images), on_step=True, on_epoch=True, prog_bar=True)
        return total_loss
    ...
    @staticmethod
    def fastsiam_loss(prediction_branch_pred: torch.Tensor, target_branch_target: torch.Tensor) -> torch.Tensor:
