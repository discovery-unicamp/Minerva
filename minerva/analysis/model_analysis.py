...
class ModelAnalysis:
    ...
    Note that, differently from `Metric`, `ModelAnalysis` does not return any
    value. Instead, the results of the analysis should be saved in the `path`
    directory. All subclasses of `ModelAnalysis` should implement the `compute`
...
class TSNEAnalysis(ModelAnalysis):
